"""Fast path integral for new SINGLE seeds 51-54 + existing data."""
...
for seed in SEEDS:
    ...
    for step in CKPT_STEPS:
        ...
        mus.append(mu)
        print(f'  seed{seed} step={step}: {np.round(mu, 4)}', flush=True)
    results[seed] = mus

# Save
with open('path_data/single_new.json', 'w') as f:
    json.dump({str(k): [m.tolist() for m in v] for k, v in results.items()}, f, indent=2)

# Compute path integrals
print('\n=== Path Integrals ===')
for seed, mus in results.items():
    mus = np.array(mus)
    path_len = np.sum(np.linalg.norm(np.diff(mus, axis=0), axis=1))
    endpt = np.linalg.norm(mus[-1] - mus[0])
    curv = path_len / max(endpt, 1e-6)
    print(f'  SINGLE seed{seed}: path={path_len:.4f} endpt={endpt:.4f} curv={curv:.1f}x')
