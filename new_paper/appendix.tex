...
\title{Technical Supplement:\\When Learning Paths Lie -- Gradient Retention under Hidden Relations}
...
\item When $\|g_A\|^2 + \|g_B\|^2 = 0$, report zero gradient energy and
  $\kappa=\mathrm{N/A}$. There is no signal to retain, so a numerical ratio is
  not identifiable.
\item When exactly one conditional gradient is zero, $\kappa=0.5$ by algebra.
  This is one-sided gradient energy, not evidence that the gradients are
  orthogonal.
\end{itemize}
...
at least coarse-partition energy. Since the retained numerator is unchanged,
...
environment and policy. Cross-algorithm values must therefore name the
estimator; an empirical reversal is not a proof that either family is immune to
hidden information.
...
\bottomrule
\end{tabular}
\caption{Complete stored dataset across environments, algorithms, and modes.
The PPO Overcooked \textsc{Dynamic} cell has zero conditional-gradient
energy, so its retention ratio is undefined rather than evidence of
cancellation.}
\end{table}
...
\begin{tabular}{lcc}
\toprule
Reveal Fraction & Split-batch ratio (mean $\pm$ std) & $n$ \\
\midrule
0\%   & $0.534 \pm 0.038$ & 2 \\
25\%  & $0.446 \pm 0.085$ & 2 \\
50\%  & $0.514 \pm 0.024$ & 2 \\
...
\bottomrule
\end{tabular}
\caption{Two independent rollout batches under each reveal condition
(510K, PPO). These ratios audit sampling consistency; because the batches
do not index distinct latent relations, they are not relation-conditioned
retention estimates.}
\end{table}
...
\noindent\textbf{Hardware:} NVIDIA GeForce RTX 5070 Ti Laptop GPU
(12 GB VRAM), Intel(R) UHD Graphics (auxiliary), and 32 GB RAM.
\textbf{Software:} Windows 11; PyTorch 2.x; stable-baselines3 and
sb3-contrib 2.x; Gymnasium 0.29+; NumPy 1.24+.
...
\item \textbf{Reproduction package:}
  \url{https://github.com/flavieninpekin/AAAI2027-510k-clear}
\item \textbf{510K environment:}
  \url{https://github.com/flavieninpekin/510k_env}
\end{itemize}

The reproduction package contains pre-computed validation results, all
experiment configurations, and the entry point experiments/reproduce\_all.py.
